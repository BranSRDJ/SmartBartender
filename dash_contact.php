...
<nav id="Sidebar" class="col-md-2 d-none d-md-block bg-light sidebar">
      <div class="sidebar-sticky">
		...
		<h6 class="sidebar-heading d-flex justify-content-between align-items-center px-3 mt-4 mb-1 text-muted">
          ...
          <li class="nav-item">
            <a class="nav-link active" href="dash_contact.php">
              <span data-feather="file"></span>
             Manage Contact <span class="sr-only">(current)</span>
            </a>
          </li>
          <li class="nav-item">
...
$qry = mysqli_query($db,"select * from bar_contact where id ='". $_POST["id"]."'");
...
echo $rs["id"];
 ...
 ?>"/>
    ...
    <tr>
      <td>Name</td>
      <td colspan="2" ><label>
        <input name="nume" size="35" type="text" id="nume" value="<?php 
if($_POST["Search"]=="Search"){
$db=mysqli_connect("localhost","root","");
mysqli_select_db($db,"smart_bartender");
$qry = mysqli_query($db,"select * from bar_contact where id ='". $_POST["id"]."'");
...
echo $rs["email"];
    ...
    <tr>
      <td>Message</td>
      <td colspan="2"><input name="mesaj" type="text" id="mesaj" size="35" value="<?php 
if($_POST["Search"]=="Search"){
$db=mysqli_connect("localhost","root","");
mysqli_select_db($db,"smart_bartender");
$qry = mysqli_query($db,"select * from bar_contact where id ='". $_POST["id"]."'");
...
mysqli_query($db,"insert into bar_contact(nume,email,mesaj)values('".$nume."','".$email."','".$mesaj."')");
echo "<center>Save successful.</center>";
}

//Update button Code
if($_POST["Update"]=="Update Record"){
mysqli_query($db,"update bar_contact set nume = '".$nume."', email = '".$email."', mesaj='".$mesaj."' where id ='".$id."'");
echo "<center>update successful.</center>";
}

//Delete button Code
if($_POST["Delete"]=="Delete Record"){
mysqli_query($db,"delete from bar_contact where id = '".$id."'");
...
$qry = mysqli_query($db,"select * from bar_contact where id ='".$id."'");
$qry2 = mysqli_query($db,"select * from bar_contact");
